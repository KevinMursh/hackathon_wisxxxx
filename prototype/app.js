...
function renderExtract() {
  ...
  const checks = c.checks.map((k) => { let st = k[2], note = k[3]; if (st === "auto") { if (!p) { st = "warn"; note = "卷內無送達證書，待補"; } else if (p.recv === null) { st = "warn"; note = "未有收文日"; } else if (p.over) { st = "fail"; note = `逾期 ${p.days} 日`; } else { st = "pass"; note = `尚餘 ${p.left} 日`; } } return `<div class="chk ${st}"><span class="mk">${marks[st]}</span><div><span>${k[1]}</span><span class="art">訴願法 ${k[0]}　${esc(note)}</span></div></div>`; }).join("");
  const over = !!(p && p.over);
  const verdict = over ? { kind: "stop", big: "應為不受理", text: `依送達證書 ${S.served} 起算，法定期間至 ${toMg(p.due)} 屆滿，機關收文日 ${S.recv} 已逾期 ${p.days} 日。程序審查於第 2 款終結，AI 判定為不受理。` } : p && p.recv !== null ? { kind: "go", big: "期間內提起", text: `收文日距屆滿尚餘 ${p.left} 日，進入實體審查。實體爭點見「爭點」分頁（共 ${c.issues.length} 個）。` } : { kind: "go", big: "待補程序資料", text: "卷內尚無送達證書或收文戳，無法完成期間計算；請補件後重新分析。" };
  let tl = "";
  ...
  $("#p0").innerHTML = `
    ...
    <div class="sec"><div class="sec-head"><h3>訴願期間計算</h3><span class="note">依卷附送達證書與收文戳自動計算；不服請提出異議</span></div><div class="box2"><div class="period-edit">
      <div><label>送達日（卷證）</label><div class="val">${esc(S.served || "—")}</div><div class="src">${c.period.servedRef ? SJ(c.period.servedRef, "來源：送達證書 ↗") : "來源：訴願書自述／待補"}${ap && ap !== S.served ? `<span class="warnsrc">　⚠ 訴願書自述 ${ap}，已採卷證</span>` : ""}</div></div>
      ...
      <div><label>期間屆滿日</label><div class="val">${p ? toMg(p.due) : "—"}</div><div class="src">起算 ${p ? toMg(p.start) : "—"}（送達次日）</div></div>
      <div><label>結果</label><div style="font-size:15px;font-weight:600;padding:4px 0;color:${over ? "var(--seal)" : "var(--green)"}">${p ? (p.recv === null ? "待補收文日" : over ? `逾期 ${p.days} 日` : `尚餘 ${p.left} 日`) : "待補送達日"}</div><div class="src">在途期間未扣除</div></div>
    ...
    <p class="foot-note">三方對照三欄之資料分別限於對應來源之文件；「卷證事實」欄優先採第三方（郵務機關、檢舉人影像、監理機關）證據。紅底列為主張與卷證不一致處，系統不自動裁決。期間以卷附送達證書為準，承辦人不服請於草稿頁提出異議。</p>`;
}
