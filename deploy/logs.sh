...
if [ -z "$UNIT" ]; then CMD="for u in app-node analysis; do echo \"===== \$u (\$(systemctl is-active \$u))\"; journalctl -u \$u --no-pager -n $N -o short-iso | cut -c1-220; done"
elif [ "$N" = "grep" ]; then CMD="journalctl -u $UNIT --no-pager -n 2000 -o short-iso | grep -E '$PAT' | tail -80 | cut -c1-240"
else CMD="echo \"===== $UNIT (\$(systemctl is-active $UNIT))\"; journalctl -u $UNIT --no-pager -n $N -o short-iso | cut -c1-240"; fi
